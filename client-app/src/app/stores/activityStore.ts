...
class ActivityStore {
    ...
    groupActivityByDate(activities: IActivity[]) {
        const sortingActivities = activities.sort(
            (a, b) => Date.parse(a.date) - Date.parse(b.date)
        )
        return Object.entries(sortingActivities.reduce((activities, activity) => {
    ...
    @action loadActivity = async (id: string) => {
        ...
        if (activity) {
            ...
            try {
                ...
                runInAction('deteils activity', () => {
                    this.activity = activity;
                    this.loadingInitial = false;
                })
            } catch(error) {
                runInAction('details activity error',() => {
                    this.loadingInitial = false;
                })
                console.log(error);
            }
